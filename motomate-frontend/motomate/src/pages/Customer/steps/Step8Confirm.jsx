...
import { Car, Wrench, CalendarDays, MapPin, Home, Zap, FileText, Building2 } from 'lucide-react';
import { StepHeader } from '../components/SharedUI';

const Step8 = ({ formData, onConfirm, onEdit }) => {
    ...
    const serviceNames = serviceObjects.map(s => s.name).join(', ') || 'None selected';

    const rows = [
        { label: 'Vehicle',         value: `${formData.vehicleType} · ${vehicleName}`,                                                 icon: <Car size={16} /> },
        { label: 'Service Center',  value: formData.serviceCenterName || '—',                                                          icon: <Building2 size={16} /> },
        { label: 'Services',        value: serviceNames,                                                                               icon: <Wrench size={16} /> },
            ...
            <div className="rounded-2xl border border-slate-200 overflow-hidden divide-y divide-slate-100">
                ...
                <div className="flex justify-between items-center px-5 py-4 bg-blue-600">
                    ...
                    <span className="text-white text-xl font-bold">
                        {grandTotal > 0 ? `₹${grandTotal}` : 'TBD by center'}
                    </span>
                </div>
            </div>
