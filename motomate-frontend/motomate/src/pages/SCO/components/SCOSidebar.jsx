...
import {
  ...
  UserCircle2, Car, ChevronRight, X, LogOut, AlertTriangle
} from 'lucide-react';

const SCOSidebar = ({ mobileOpen, onClose, onLogout, centerName, sosBadge = 0, onSosBadgeClear }) => {
  const NAV = [
    {
      section: 'Overview',
      links: [
    ...
    },
    {
      ...
      links: [
        ...
        {
          to: '/dashboard/service-center-owner/sos',
          label: 'SOS Alerts',
          icon: AlertTriangle,
          badge: sosBadge,
          onActivate: onSosBadgeClear,
          emergency: true,
        },
      ],
    },
          ...
          {NAV.map(({ section, links }) => (
            <div key={section}>
              ...
              <div className="space-y-0.5">
                {links.map(({ to, label, icon: Icon, end, badge, onActivate, emergency }) => (
                  <NavLink
                    key={to}
                    to={to}
                    end={end}
                    ...
                    className={({ isActive }) =>
                      ...
                      ${isActive
                        ...
                        : emergency && badge > 0
                          ? 'text-white bg-red-500/80 hover:bg-red-500 animate-pulse'
                  ...
                  >
                    {({ isActive }) => (
                      <>
                        ...
                        {/* SOS badge */}
                        {badge > 0 && !isActive && (
                          <span className="w-5 h-5 rounded-full bg-white text-red-600 text-[10px] font-black flex items-center justify-center">
                            {badge > 9 ? '9+' : badge}
                          </span>
                        )}
